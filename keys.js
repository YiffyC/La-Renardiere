...
document.onkeydown = function(e) {
	...
	if ( key == 32 ) { // Key space, focus search bar and show [ESP] instruction.
		...
		helpToggle();
	} else if ( key == 38 || key == 90) { // Up key, go back 4 blocks (the one above).
		...
		result = !isNaN(document.activeElement.id) ? result : focused;
	} else if ( key == 37 || key == 81 ) { // left key, go back 1 block or reset row if end.
